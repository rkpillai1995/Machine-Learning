...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        # Fifth layer in the CNN, which is fully connected layer. It takes in an
        # input of 120 nodes and gives output of 84 nodes.
        self.fc3 = nn.Linear(120, 84)

        # Sixth and last layer in the CNN, which is fully connected layer.
        # It takes in an input of 84 nodes and gives 10 output values.
        # Once the network is trained, these 10 values will be the probabilities
        # of the input audio sample with the corresponding class in belongs to.
    ...
    def forward(self, x):
        ...
        # Input put through the second convolution layer
        x = F.relu(self.conv2(x))

        # Max pooling the output of the second Convolution layer. This is a
        # max-pool of kernal size 2. This way, the input audio signal will be
        # reduced by half.
        x = F.max_pool1d(x, 2)
        ...
        # Fifth layer in the CNN, which is fully connected
        x = F.relu(self.fc3(x))

        # Last layer in the CNN, which is again fully connected
        x = self.fc4(x)
...
def train_network(network, data_set, optimizer, criterion, start_ranges, end_ranges, device="cpu"):
    ...
    for i, data in enumerate(data_set, 0):
        ...
        inputs, instrument_source_target = inputs.to(device, dtype=torch.float), instrument_source_target.to(device)
        ...
        loss = criterion(outputs, instrument_source_target)
...
def predict_output(network, data_set, optimizer, criterion, start_ranges, end_ranges, device="cpu"):
    ...
    for i, data in enumerate(data_set, 0):
        ...
        prediction_loss = criterion(outputs, instrument_source_target)

        # class prediction
        _, predicted_class = torch.max(outputs.data, 1)

        # Counting the total number of correct prediction
        total_correct_predictions += (predicted_class == instrument_source_target).sum().item()
...
def classify(network, data_set, start_ranges, end_ranges, device="cpu", location="./"):
    ...
    correct_classification_audio_sample = [None] * 3

    # variables to remember the lowest probability of the wrong classification
    # for each class, and it's corresponding audio sample
    incorrect_classification = [1] * 3
    incorrect_classification_audio_sample = [None] * 3
    ...
    slightly_higher_classification = [0] * 3
    slightly_higher_classification_audio_sample = [None] * 3

    # variables to remember the lower probability of the wrong classification
    # near the decision boundary for each class, and it's corresponding audio sample
    slightly_lower_classification = [0] * 3
    slightly_lower_classification_audio_sample = [None] * 3
    ...
    for i, data in enumerate(data_set, 0):
        ...
        # Moving the sub sampled array to GPU
        images_in_device, instrument_source_target = audio_samples.to(device, dtype=torch.float), instrument_source_target.to(device)
        ...
        for index in range(len(cpu_prediction)):
            ...
            predicted_class, prediction_probability = max(enumerate(prediction_probabilities),
                                                          key=operator.itemgetter(1))

            # Actual class of the current data sample
            actual_class = int(instrument_source_target[index])
            ...
            if actual_class == predicted_class:
                ...
                if slightly_higher_classification[actual_class] < second_max_probability:
                    ...
                    second_max_class = \
                    sorted(range(len(prediction_probabilities)), key=lambda k: prediction_probabilities[k])[1]

                    # Record the predicted probability, missed probability and the class which we just avoided
                    slightly_higher_classification_audio_sample[actual_class] = [prediction_probability,
                                                                                 second_max_probability,
                                                                                 second_max_class,
                                                                                 original_audio_samples[index]]
            ...
            # If the prediction is incorrect
                ...
                if incorrect_classification[actual_class] > prediction_probabilities[actual_class]:
                    ...
                    incorrect_classification_audio_sample[actual_class] = [predicted_class,
                                                                           original_audio_samples[index]]
                ...
                if slightly_lower_classification[actual_class] < prediction_probabilities[actual_class]:
                    ...
                    slightly_lower_classification_audio_sample[actual_class] = [prediction_probability,
                                                                                prediction_probabilities[actual_class],
                                                                                predicted_class,
                                                                                original_audio_samples[index]]
    ...
    plot_border_audio_samples(slightly_higher_classification_audio_sample, slightly_lower_classification_audio_sample,
                              location)
...
def plot_border_audio_samples(slightly_higher_classification_data, slightly_lower_classification_data, location="./"):
    ...
    for correctClassification in slightly_higher_classification_data:
        ...
        prediction_probability, second_max_probability, second_max_class, audio_sample = correctClassification[0], \
                                                                                         correctClassification[1], \
                                                                                         correctClassification[2], \
                                                                                         correctClassification[3]
        fig, ax = plt.subplots()
        image_title = "Class - " + str(index) + " (" + str(prediction_probability) + ") Missed Class - " + str(
            second_max_class) + " (" + str(second_max_probability) + ")"
    ...
    for incorrectClassification in slightly_lower_classification_data:
        ...
        prediction_probability, actual_class_probability, predicted_class, audio_sample = incorrectClassification[0], \
                                                                                          incorrectClassification[1], \
                                                                                          incorrectClassification[2], \
                                                                                          incorrectClassification[3]
        fig, ax = plt.subplots()
        image_title = "Class - " + str(index) + " (" + str(actual_class_probability) + ") Predicted Class - " + str(
            predicted_class) + " (" + str(prediction_probability) + ")"
...
def create_confusion_matrix_image(confusion_matrix, class_counts, location="./"):
    ...
    for actualLabel in range(len(confusion_matrix)):
        for predictedLabel in range(len(confusion_matrix[0])):
            normalized_matrix[actualLabel, predictedLabel] = confusion_matrix[actualLabel, predictedLabel] / \
                                                             class_counts[actualLabel]
...
def main():
    ...
    weights_file_name = "q4 weights.txt"
    try:
    ...
    for epoch in range(epochs):
        ...
        validation_loss, correct = predict_output(net, validation_loader, optimizer, criterion, start_range, end_range,
                                                  device)
